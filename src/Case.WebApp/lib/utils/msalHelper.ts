import { loginRequest, msalInstance } from "@/lib/msalConfig";

export async function acquireGraphAccessToken() {
  const account = msalInstance.getActiveAccount();
  if (!account) {
    throw new Error(
        "No active account! Verify a user has been signed in and setActiveAccount has been called."
    );
  }

  try {
    const response = await msalInstance.acquireTokenSilent({
      ...loginRequest,
      account: account,
    });
    return response.accessToken;
  } catch (error) {
    if (
        error instanceof Error &&
        (error.message.includes("AADSTS50058") || error.message.includes("AADSTS50076"))
    ) {
      // Attempt to acquire the token via redirect
      msalInstance.acquireTokenRedirect(loginRequest);
    } else {
      console.error("Failed to acquire token silently", error);
      throw error; // Rethrow if it's a different error
    }
  }
}

export function handleSignIn() {
  msalInstance.loginRedirect(loginRequest).catch((e) => {
    console.error(`loginRedirect failed: ${e}`);
  });
}

export function handleSignOut() {
  const logoutRequest = {
    account: msalInstance.getActiveAccount(),
    postLogoutRedirectUri: "/", // Define where the user should be redirected post-logout
  };

  msalInstance.logoutRedirect(logoutRequest).catch((e) => {
    console.error(`logoutRedirect failed: ${e}`);
  });
}